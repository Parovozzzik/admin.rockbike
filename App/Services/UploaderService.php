...
class UploaderService
{
    ...
    private function validate($file = null)
    {
        ...
        if (!empty($errors)) {
            ...
            if (!isset($this->data['errors'])) $this->data['errors'] = [];
    ...
    private function prepareFiles()
    {
        ...
        if ($validate) {
            ...
            for ($i = 0; $i < count($field['name']); $i++) {
                ...
                $metas['name'] = $this->options['fileName'];
                ...
                if (!$is_file_removed && $this->validate(array_merge($metas, ['index' => $i, 'tmp' => $tmp_name])) && $this->uploadFile($tmp_name, $metas['file'])) {
